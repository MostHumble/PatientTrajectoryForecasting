...
def get_admission_stats(subject_id_adm_map, num_visits=15):
    ...
    num_admissions_less = [
        len(admissions)
        for admissions in subject_id_adm_map.values()
        if len(admissions) < num_visits
    ]
    num_admissions_more = [
        len(admissions)
        for admissions in subject_id_adm_map.values()
...
def plot_all_distributions(steps_data, num_visits=15, save=False, dpi=600, prefix=""):
    ...
    for i, (step, (counts_less, _)) in enumerate(steps_data.items()):
        ...
        axs[0].bar(
            counts_less.keys(), counts_less.values(), color=color, alpha=0.5, label=step
        )

    axs[0].set_xlabel("Number of visits")
    axs[0].set_ylabel("Number of Subjects")
    axs[0].set_title(
        f"Distribution of Number of Subjects per Number of Visits (< {num_visits})"
    )
    ...
    for i, (step, (_, (x, cumulative))) in enumerate(steps_data.items()):
        color = colors[i % len(colors)]
        axs[1].plot(
            x, cumulative, marker="o", linestyle="-", color=color, alpha=0.5, label=step
        )

    axs[1].set_xlabel("Number of visits")
    axs[1].set_ylabel("Number of Subjects")
    axs[1].set_title(
        f"Cumulative Distribution of Number of Subjects per Number of Visits (> {num_visits})"
    )
    ...
    if save:
        plt.savefig(
            f"./stats/figures/distribution_subject_num_visits_combined_{prefix}.png",
            dpi=dpi,
        )
    plt.show()


def plot_admission_distribution(
    subject_id_adm_map,
    num_visits=15,
    save=False,
    dpi=600,
    ...
    # Plot the first distribution
    axs[0].bar(
        counts_less.keys(), counts_less.values(), color="green"
    )  # Change color to blue for all bars
    axs[0].bar(
        1, counts_less.get(1, 0), color="red"
    )  # Change color to red for the first bar
    axs[0].set_xlabel("Number of visits")
    axs[0].set_ylabel("Number of Subjects")
    axs[0].set_title(
        f"Distribution of Number of Subject per Number of Visits ({num_visits} <)"
    )
    axs[0].set_xlim(0, num_visits)
    axs[0].set_xticks(range(1, num_visits))
    axs[0].set_yticks([])
    ...
    axs[1].set_ylabel("Number of Subjects")
    axs[1].set_title(
        f"Cumulative distribution of Number of Subject per Number of Visits (> {num_visits} )"
    )
    ...
    if save:
        plt.savefig(
            f"./stats/figures/distribution_subject_num_visits_{prefix}_{num_visits}.png",
            dpi=dpi,
        )
    plt.show()
