...
def evaluate(model, Loader, criterion, device):
    ...
    with torch.no_grad():
        for batch in Loader:
            src, trg = batch["source_sequences"].to(device), batch[
                "target_sequences"
            ].to(device)
...
if __name__ == "__main__":
    ...
    reverseOutTypes = {
        v: source_tokens_to_ids[k] for k, v in target_tokens_to_ids.items()
    }
    ...
    gen = Generator(enc, dec, src_pad_idx=SRC_PAD_ID, trg_pad_idx=TARGET_PAD_ID).to(
        device
    )
    ...
    disc = Discriminator(
        ...
        max_length=MAX_INPUT_LEN + MAX_OUT_LEN,
    ).to(device)
    ...
    lr_schedulerG = NoamLR(
        gen_opt, warmup_steps=WARMUP_STEPS, factor=FACTOR, model_size=HID_DIM
    )
    lr_schedulerD = NoamLR(
        disc_opt, warmup_steps=WARMUP_STEPS, factor=FACTOR, model_size=HID_DIM
    )
    ...
    criterion = LabelSmoothingCrossEntropy(
        epsilon=LABEL_SMOOTHING, ignore_index=TARGET_PAD_ID
    )
    ...
    for epoch in range(0, n_epochs):
        ...
        for batch in tqdm(trainLoader):
            ...
            for _ in range(crit_repeats):
                ...
                output, _ = gen(
                    src, trg[:, :-1]
                )  # encoder-decoder returns output, attention
        ...
        print(
            f" Train loss {totalGen/len(trainLoader)} , validation loss :{valid_loss}"
        )
