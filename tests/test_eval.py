...
def test_get_sequences(
    pred_trgs: List[List[int]],
    targets: List[List[int]],
    source_input_ids: Tensor,
    test_dataloader: DataLoader,
    config: dataclass,
    tgt_tokens_to_ids: Dict[str, int],
    max_lenght: int,
):
    ...
    assert len(pred_trgs) == len(
        targets
    ), "Mismatch in number of predictions and targets"
    assert (len(pred_trgs)) == config.eval_batch_size * (
        len(test_dataloader) - 1
    ) + source_input_ids.size(0), "Not all sequences have been generated"
