...
def train_transformer(
    ...
    if with_notes:
        transformer = Seq2SeqTransformerWithNotes(
            ...
            config.emb_size,
            config.nhead,
            data_config.source_vocab_size,
            data_config.target_vocab_size,
            config.ffn_hid_dim,
            dropout,
            positional_encoding,
        )
        ...
        transformer = Seq2SeqTransformer(
            config.num_encoder_layers,
            config.num_decoder_layers,
            config.dim_per_head * config.nhead,
            config.nhead,
            data_config.source_vocab_size,
            data_config.target_vocab_size,
            config.ffn_hid_dim,
    ...
    loss_fn = torch.nn.CrossEntropyLoss(
        ignore_index=data_config.target_pad_id, label_smoothing=config.label_smoothing
    )
    ...
    for epoch in range(config.num_train_epochs):
        ...
        pred_trgs, targets = get_sequences(
            transformer,
            val_dataloader,
            data_config.source_pad_id,
            tgt_tokens_to_ids,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dim_per_head", type=int, default=64, help="Dimension per head"
    )
    ...
    parser.add_argument(
        "--label_smoothing", type=float, help="Label smoothing (min 0, max 0.2)"
    )
    parser.add_argument(
        "--learning_rate", type=float, help="Learning rate (min 5e-05, max 0.008)"
    )
    ...
    parser.add_argument(
        "--scheduler",
        type=str,
        choices=["ReduceLROnPlateau", "CosineAnnealingWarmRestarts"],
        help="Type of scheduler",
    )

    # Quantized log uniform distribution parameters (handled as int for simplicity)
    parser.add_argument(
        "--train_batch_size", type=int, help="Training batch size (min 32, max 64)"
    )
    ...
    (
        ...
        eval_batch_size=args.train_batch_size * 2,
        pin_memory=True,
        **asdict(data_config),
    )
