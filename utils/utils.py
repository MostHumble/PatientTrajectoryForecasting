...
def load_data(path: str = 'outputData/originalData/' , updated_ids_to_types : bool = False,
        ...
        if train:
            ...
            if with_notes:
                subject_id_hadm_map = pickle.load(open(os.path.join(path, 'subject_id_hadm_map.pkl'), 'rb'))
            if updated_ids_to_types:
                ...
                if with_notes:
                    return source_target_sequences, ids_to_types_map, tokens_to_ids_map, updated_ids_to_types, reverse_up_ids_to_types, subject_id_hadm_map
                return source_target_sequences, ids_to_types_map, tokens_to_ids_map, updated_ids_to_types, reverse_up_ids_to_types
            if with_notes:
                return source_target_sequences, ids_to_types_map, tokens_to_ids_map, ids_to_tokens_map, subject_id_hadm_map
...
def get_paths(config: dict, strategy : str = None, predict_procedure : bool = False, predict_drugs : bool = False,
    ...
    if train:
        if predict_procedure and predict_drugs :
            paths['train_data_path'] = os.path.join(root, output_file, strategy, output_files['all_outputs_file'][:-1] + suffix)
        if predict_procedure and not(predict_drugs):
            paths['train_data_path'] = os.path.join(root, output_file, strategy, output_files['diagnosis_procedure_output_file'][:-1] + suffix)
        else:
            paths['train_data_path'] = os.path.join(root, output_file, strategy, output_files['diagnosis_output_file'][:-1] + suffix)
        if processed_data:
            paths['processed_data_path'] = os.path.join(root, paths['train_data_path'], 'processed_data' + suffix)
        return paths
...
def store_files(source_target_sequences : List[Tuple[List[int], List[int]]] = None,
    ...
    if train:
        ...
        if subject_id_hadm_map is not None:
                pickle.dump(subject_id_hadm_map, open(os.path.join(output_file, 'subject_id_hadm_map.pkl'), 'wb'), -1)
    ...
    if processed_data:
        ...
        if hospital_ids_source is not None:
            pickle.dump(hospital_ids_source, open(output_file + 'hospital_ids_source.pkl', 'wb'), -1)
