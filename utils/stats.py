...
def stats(
    Pairs: List[Tuple[List[List[int]]]], is_torch: bool = False
    ...
    print(
        f"Mean sequence length of target sequences : {mean_target:.2f} ± {std_target:.2f}"
    )
    ...
    plt.hist(
        ...
        bins=30,
        color=["blue", "green"],
        label=["source sequences", "target sequences"],
    )
...
def token_count_distributions(
    source_lenghts: List[int],
    target_lenghts: List[int],
    bins: int = 100,
    xlim: int = 50,
) -> None:
    """
...
def show_lost(
    ...
    print(
        f"Number of sequences lost assuming an average sequence lenght of  {np.mean(target_lenghts) :0.2f} Target: {target_sequences:0.2f}"
    )
    print(
        f"Number of sequences lost assuming an average sequence lenght of  {np.mean(source_lenghts) :0.2f} Source : {source_sequences:0.2f}"
    )
