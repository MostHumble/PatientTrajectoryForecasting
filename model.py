...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, token_embedding: torch.Tensor):
        return self.dropout(token_embedding + self.pos_embedding[:token_embedding.size(1)]) # seq_len
...
class Seq2SeqTransformerWithNotes(nn.Module):
    def __init__(self,
        ...
        if self.bert.config.strategy == 'all':
            self.projection = LinearProjection(bert.config.seq_len, bert.config.seq_len // 2) # seq_len
    ...
    def forward(self,
        ...
        if self.bert:
            ...
            # out_dim : batch, num_encoder_layers, seq_len, hidden dim
                ...
                out_notes = self.projection(out_notes) # num_layers, batch_size (concatenated hospital admissions), hidden_dim
                batch_embeddings = []
                start_idx = 0
                ...
                for i,length in enumerate(hospital_ids_lens):
                    ...
                    if self.use_positional_encoding_notes:
                        batch_embeddings.append(torch.cat([self.positional_encoding(out_notes[:,start_idx:start_idx + length]).reshape(-1,768), source_embeds[i]], dim = 0)[:512,])
                    else:
                        batch_embeddings.append(torch.cat([out_notes[:,start_idx:start_idx + length].reshape(-1,768), source_embeds[i]], dim = 0)[:512,])
                    start_idx += length
    ...
    def encode(self, src: torch.Tensor, src_mask: torch.Tensor, notes_input_ids: torch.Tensor, notes_attention_mask: torch.Tensor, notes_token_type_ids: torch.Tensor, hospital_ids_lens: torch.Tensor):
    ...
    def batch_encode(self,
        ...
        return self.transformer.encoder(source_embeds,  mask=src_mask, src_key_padding_mask=src_key_padding_mask)
...
class LinearProjection(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        """
        permuted_tensor = input_tensor.permute(0, 1, 3, 2)  # (num_layers, batch_size, input_dim, seq_len)
        
        hidden_tensor = self.input_to_hidden(permuted_tensor)  # (num_layers, batch_size, hidden_dim, seq_len)
        
        activated_tensor = self.activation(hidden_tensor)  #  (num_layers, batch_size, hidden_dim, seq_len)
        
        output_tensor = self.hidden_to_output(activated_tensor)  # (num_layers, batch_size, hidden_dim, 1)
        
        output_tensor = output_tensor.squeeze(-1)  # (num_layers, batch_size, hidden_dim)
