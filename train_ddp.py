...
def train_transformer(
    ...
    for epoch in range(args.num_train_epochs):
        ...
        dist.barrier()
        logging.info(
            f'Epoch: {epoch}, Train Loss: {train_loss}, LR: {optimizer.param_groups[0]["lr"]}, Device: {DEVICE}'
        )
        if DEVICE == 0:
            wandb.log(
                {
            ...
            if epoch % args.eval_every == 0 and epoch > 0:
                ...
                if pred_trgs:
                    test_mapk = {
                        f"test_map@{k}": mapk(targets, pred_trgs, k) for k in ks
                    }
                    test_recallk = {
                        f"test_recall@{k}": recallTop(targets, pred_trgs, rank=[k])[0]
                        for k in ks
                    }
                    wandb.log(
                        {"val_loss": val_loss, **test_mapk, **test_recallk}, step=epoch
                    )
                    print(
                        f'test_map@20: {test_mapk["test_map@20"]}, test_map@40: {test_mapk["test_map@40"]}, test_map@60: {test_mapk["test_map@60"]}, test_recall@20: {test_recallk["test_recall@20"]}, test_recall@40: {test_recallk["test_recall@40"]}, test_recall@60: {test_recallk["test_recall@60"]}',
                        flush=True,
                    )
...
if __name__ == "__main__":
    ...
    if local_rank == 0:
        wandb.init(
            ...
            project="PTF_SDP_D_NOTES_LONGER",
            config=args,
        )
